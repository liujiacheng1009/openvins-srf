...
public:
   ...
   */
  static std::vector<core::ImuData> select_imu_readings(const std::vector<core::ImuData> &imu_data_tmp, double time0, double time1) {
    ...
    for (size_t i = 0; i < imu_data_tmp.size() - 1; i++) {
      ...
      if (imu_data_tmp.at(i + 1).timestamp > time0 && imu_data_tmp.at(i).timestamp < time0) {
        core::ImuData data = interpolate_data(imu_data_tmp.at(i), imu_data_tmp.at(i + 1), time0);
      ...
      if (imu_data_tmp.at(i + 1).timestamp > time1) {
        if (imu_data_tmp.at(i).timestamp > time1 && i == 0) {
          ...
          core::ImuData data = interpolate_data(imu_data_tmp.at(i - 1), imu_data_tmp.at(i), time1);
          prop_data.push_back(data);
        } else {
          prop_data.push_back(imu_data_tmp.at(i));
        }
        if (prop_data.at(prop_data.size() - 1).timestamp != time1) {
          core::ImuData data = interpolate_data(imu_data_tmp.at(i), imu_data_tmp.at(i + 1), time1);
          prop_data.push_back(data);
        }
        break;
      }
   ...
   * @param frame_orientation Orientation of the imu frame respect to the body frame
   */
  static void gram_schmidt(const Eigen::Matrix<number_t, 3, 1> &gravity_inI, Eigen::Matrix<number_t, 3, 3> &R_GtoI,
                           const std::string &frame_orientation)
  {
    ...
    Eigen::Matrix<number_t, 3, 1> z_axis = gravity_inI / gravity_inI.norm();
    Eigen::Matrix<number_t, 3, 1> x_axis, y_axis;
    Eigen::Matrix<number_t, 3, 1> e_1(1.0, 0.0, 0.0);
    Eigen::Matrix<number_t, 3, 1> e_2(0.0, 1.0, 0.0);
    number_t inner1 = e_1.dot(z_axis) / z_axis.norm();
    number_t inner2 = e_2.dot(z_axis) / z_axis.norm();
    ...
    // x_axis = x_axis / x_axis.norm();
    // y_axis = core::skew_x(z_axis) * x_axis;
    // y_axis = y_axis / y_axis.norm();
    ...
    number_t pi = static_cast<number_t>(M_PI);
    if (frame_orientation == "XYZ" && fabs(inner1) < fabs(inner2))
      R_GtoI = R_GtoI * Eigen::AngleAxis(-pi / 2, Eigen::Matrix<number_t, 3, 1>::UnitZ());
    else if (frame_orientation == "XYZ" && fabs(inner1) >= fabs(inner2))
      R_GtoI = R_GtoI * Eigen::AngleAxis(-pi, Eigen::Matrix<number_t, 3, 1>::UnitZ());
    else if (frame_orientation == "X-ZY")
      R_GtoI = R_GtoI * Eigen::AngleAxis(-pi / 2, Eigen::Matrix<number_t, 3, 1>::UnitZ());
    else
      PRINT_ERROR(RED "unknown frame orientation!!!\n" RESET);
  }
