...
bool StaticInitializer::initialize(double &timestamp, MatrixX &covariance, std::vector<std::shared_ptr<Type>> &order,
                                   std::shared_ptr<IMU> t_imu, const std::string &frame_orientation) {
  ...
  std::vector<ImuData> window_2to1;
  for (const ImuData &data : *imu_data)
  {
    if (data.timestamp > newesttime - 1.0 * params.init_window_time && data.timestamp <= newesttime - 0.0 * params.init_window_time)
    {
  ...
  Eigen::Matrix<number_t, 3, 1> a_avg_2to1 = Eigen::Matrix<number_t, 3, 1>::Zero();
  Eigen::Matrix<number_t, 3, 1> w_avg_2to1 = Eigen::Matrix<number_t, 3, 1>::Zero();
  ...
  Eigen::Matrix<number_t, 3, 1> z_axis = a_avg_2to1 / a_avg_2to1.norm();
  Eigen::Matrix<number_t, 3, 3> Ro;
  InitializerHelper::gram_schmidt(z_axis, Ro, frame_orientation);
  Eigen::Matrix<number_t, 4, 1> q_GtoI = rot_2_quat(Ro);

  // Set our biases equal to our noise (subtract our gravity from accelerometer bias)
  Eigen::Matrix<number_t, 3, 1> gravity_inG;
  gravity_inG << 0.0, 0.0, params.gravity_mag;
  Eigen::Matrix<number_t, 3, 1> bg = w_avg_2to1;
  Eigen::Matrix<number_t, 3, 1> ba = a_avg_2to1 - quat_2_Rot(q_GtoI) * gravity_inG;

  // Set our state variables
  timestamp = window_2to1.at(window_2to1.size() - 1).timestamp;
  VectorX imu_state = VectorX::Zero(16);
  ...
  covariance = std::pow(0.02, 2) * MatrixX::Identity(t_imu->size(), t_imu->size());
  covariance.block(0, 0, 3, 3) = std::pow(0.02, 2) * Eigen::Matrix<number_t, 3, 3>::Identity(); // q
  covariance.block(3, 3, 3, 3) = std::pow(0.05, 2) * Eigen::Matrix<number_t, 3, 3>::Identity(); // p
  covariance.block(6, 6, 3, 3) = std::pow(0.01, 2) * Eigen::Matrix<number_t, 3, 3>::Identity(); // v (static)
  ...
  // Eigen::Matrix<number_t, 3,3> R_GtoI = quat_2_Rot(q_GtoI);
  // covariance(2, 2) = std::pow(1e-4, 2);
  // covariance.block(0, 0, 3, 3) = R_GtoI * covariance.block(0, 0, 3, 3) * R_GtoI.transpose();
  // covariance.block(3, 3, 3, 3) = std::pow(1e-3, 2) * Eigen::Matrix<number_t, 3,3>::Identity();
