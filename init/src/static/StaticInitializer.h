...
public:
   ...
   */
  explicit StaticInitializer(InertialInitializerOptions &params_, std::shared_ptr<core::FeatureDatabase> db,
                             std::shared_ptr<std::vector<core::ImuData>> imu_data_)
   ...
   * @param[in] frame_orientation Orientation of the imu frame respect to the body frame
   * @return True if we have successfully initialized our system
   */
  bool initialize(double &timestamp, MatrixX &covariance, std::vector<std::shared_ptr<type::Type>> &order,
                  std::shared_ptr<type::IMU> t_imu, const std::string &frame_orientation);
...
private:
  ...
  std::shared_ptr<std::vector<core::ImuData>> imu_data;
};
