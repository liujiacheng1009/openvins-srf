...
struct InertialInitializerOptions {
  ...
  /// The installation method of the IMU relative to the glass (e.g. "x<-->right, y<-->front, z<-->up")
  std::string frame_orientation = "XYZ";
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_initializer(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("frame_orientation", frame_orientation);
    }
    ...
    PRINT_DEBUG("  - frame_orientation: %s\n", frame_orientation.c_str());
  }
