...
InertialInitializer::InertialInitializer(InertialInitializerOptions &params_, std::shared_ptr<core::FeatureDatabase> db)
    : params(params_), _db(db) {

  // Vector of our IMU data
  imu_data = std::make_shared<std::vector<core::ImuData>>();

  // Create initializers
  init_static = std::make_shared<StaticInitializer>(params, _db, imu_data);
}

void InertialInitializer::feed_imu(const core::ImuData &message, double oldest_time) {
...
bool InertialInitializer::initialize(double &timestamp, MatrixX &covariance, std::vector<std::shared_ptr<type::Type>> &order,
                                     std::shared_ptr<type::IMU> t_imu) {
  ...
  if (params.init_max_disparity > 0)
  {
    ...
    double newest_time_allowed = newest_cam_time - 1.0 * params.init_window_time;
    int num_features1 = 0;
    double avg_disp1;
    double var_disp1;
  ...
  bool is_still = !disparity_detected_moving_2to1;
  if (is_still && params.init_imu_thresh > 0.0)
  {
    PRINT_DEBUG(GREEN "[init]: USING STATIC INITIALIZER METHOD!\n" RESET);
    return init_static->initialize(timestamp, covariance, order, t_imu, params.frame_orientation);
  } else if (params.init_dyn_use && !is_still) {
    PRINT_WARNING(YELLOW "[init]: USING DYNAMIC INITIALIZER METHOD!\n" RESET);
    return false;
  } else {
    std::string msg = "platform moving too much";
