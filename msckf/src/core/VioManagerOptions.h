...
constexpr bool kUseJointUpdate = true;
constexpr bool kUseJointAnchorChange = true;
 ...
 */
enum ZUPT_CHECK_METHOD
{
  USE_DISPARITY_ONLY = 0,
  USE_IMU_ONLY = 1,
  USE_IMU_AND_DISPARITY = 2,
  USE_IMU_OR_DISPARITY = 3
};
...
struct VioManagerOptions
{
   ...
   */
  void print_and_load(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
  ...
  init::InertialInitializerOptions init_options;
  ...
  ZUPT_CHECK_METHOD zupt_check_method = ZUPT_CHECK_METHOD::USE_IMU_ONLY;

  double zupt_check_gyro_threshold = 0.05;

  double zupt_check_acc_threshold = 0.2;

  int zupt_check_imu_cnt = 100;
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_estimator(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("zupt_check_acc_threshold", zupt_check_acc_threshold);
      parser->parse_config("zupt_check_gyro_threshold", zupt_check_gyro_threshold);
      parser->parse_config("zupt_check_imu_cnt", zupt_check_imu_cnt);
      ...
      std::string zupt_check_str = "imu_only";
      parser->parse_config("zupt_check_method", zupt_check_str);
      if (zupt_check_str == "imu_only") {
        zupt_check_method = ZUPT_CHECK_METHOD::USE_IMU_ONLY;
      } else if (zupt_check_str == "disparity_only") {
        zupt_check_method = ZUPT_CHECK_METHOD::USE_DISPARITY_ONLY;
      } else if (zupt_check_str == "imu_and_disparity") {
        zupt_check_method = ZUPT_CHECK_METHOD::USE_IMU_AND_DISPARITY;
      } else if (zupt_check_str == "imu_or_disparity") {
        zupt_check_method = ZUPT_CHECK_METHOD::USE_IMU_OR_DISPARITY;
      }else {
        PRINT_ERROR(RED "invalid zupt check method: %s\n" RESET, zupt_check_str.c_str());
        PRINT_ERROR(RED "please select a valid method: imu_only, disparity_only, imu_and_disparity\n" RESET);
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_noise(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_state(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      for (int i = 0; i < state_options.num_cameras; i++) {
        ...
        VectorX cam_calib = VectorX::Zero(8);
        ...
        Eigen::Matrix<number_t, 7, 1> cam_eigen;
        cam_eigen.block(0, 0, 4, 1) = core::rot_2_quat(T_CtoI.block(0, 0, 3, 3).transpose()).cast<number_t>();
        cam_eigen.block(4, 0, 3, 1) = -T_CtoI.block(0, 0, 3, 3).transpose().cast<number_t>() * T_CtoI.block(0, 3, 3, 1).cast<number_t>();

        // Create intrinsics model
        if (dist_model == "equidistant") {
          camera_intrinsics.insert({i, std::make_shared<core::CamEqui>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i)->set_value(cam_calib);
        } else {
          camera_intrinsics.insert({i, std::make_shared<core::CamRadtan>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i)->set_value(cam_calib);
        }
      ...
      Eigen::Matrix<number_t, 3, 3> Dw = Tw.cast<number_t>().colPivHouseholderQr().solve(Eigen::Matrix<number_t, 3, 3>::Identity());
      Eigen::Matrix<number_t, 3, 3> Da = Ta.cast<number_t>().colPivHouseholderQr().solve(Eigen::Matrix<number_t, 3, 3>::Identity());
      Eigen::Matrix<number_t, 3, 3> R_ACCtoIMU = R_IMUtoACC.transpose().cast<number_t>();
      Eigen::Matrix<number_t, 3, 3> R_GYROtoIMU = R_IMUtoGYRO.transpose().cast<number_t>();
      ...
      vec_tg << Tg.cast<number_t>().block<3, 1>(0, 0), Tg.cast<number_t>().block<3, 1>(0, 1), Tg.cast<number_t>().block<3, 1>(0, 2);
      q_GYROtoIMU = core::rot_2_quat(R_GYROtoIMU);
      q_ACCtoIMU = core::rot_2_quat(R_ACCtoIMU);
    }
    ...
    for (int n = 0; n < state_options.num_cameras; n++) {
      std::stringstream ss;
      ss << "cam_" << n << "_fisheye:" << (std::dynamic_pointer_cast<core::CamEqui>(camera_intrinsics.at(n)) != nullptr) << std::endl;
      ...
      Eigen::Matrix<number_t, 4, 4> T_CtoI = Eigen::Matrix<number_t, 4, 4>::Identity();
      T_CtoI.block(0, 0, 3, 3) = core::quat_2_Rot(camera_extrinsics.at(n).block(0, 0, 4, 1)).transpose();
  ...
  core::TrackBase::HistogramMethod histogram_method = core::TrackBase::HistogramMethod::HISTOGRAM;
  ...
  core::FeatureInitializerOptions featinit_options;
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_trackers(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      if (histogram_method_str == "NONE") {
        histogram_method = core::TrackBase::NONE;
      } else if (histogram_method_str == "HISTOGRAM") {
        histogram_method = core::TrackBase::HISTOGRAM;
      } else if (histogram_method_str == "CLAHE") {
        histogram_method = core::TrackBase::CLAHE;
      } else {
   ...
   * @param parser If not null, this parser will be used to load our parameters
   */
  void print_and_load_simulation(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
