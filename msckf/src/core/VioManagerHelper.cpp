...
void VioManager::initialize_with_gt(Eigen::Matrix<double, 17, 1> imustate) {
  ...
  state->_imu->set_value(imustate.block(1, 0, 16, 1).cast<number_t>());
  state->_imu->set_fej(imustate.block(1, 0, 16, 1).cast<number_t>());

  // Fix the global yaw and position gauge freedoms
  // TODO: Why does this break out simulation consistency metrics?
  std::vector<std::shared_ptr<type::Type>> order = {state->_imu};
  MatrixX Cov = std::pow(0.02, 2) * MatrixX::Identity(state->_imu->size(), state->_imu->size());
  Cov.block(0, 0, 3, 3) = std::pow(0.017, 2) * Eigen::Matrix<number_t, 3, 3>::Identity(); // q
  Cov.block(3, 3, 3, 3) = std::pow(0.05, 2) * Eigen::Matrix<number_t, 3, 3>::Identity();  // p
  Cov.block(6, 6, 3, 3) = std::pow(0.01, 2) * Eigen::Matrix<number_t, 3, 3>::Identity();  // v (static)
  FilterWrapper::set_initial_covariance(state, Cov, order);
  FilterWrapper::set_initial_last_clone_pose(state); // only work for srif 
  // Set the state time
...
bool VioManager::try_to_initialize(const core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    double timestamp;
    MatrixX covariance;
    std::vector<std::shared_ptr<type::Type>> order;
    ...
    bool success = initializer->initialize(timestamp, covariance, order, state->_imu);
    ...
    if (success) {
      ...
      FilterWrapper::set_initial_covariance(state, covariance, order);
      FilterWrapper::set_initial_last_clone_pose(state); // only work for srif 
      // Set the state time
      ...
      Eigen::Matrix<number_t, 4,1> init_q = state->_imu->quat();
      Eigen::Quaternion<number_t> init_quat(init_q[3], init_q[0], init_q[1], init_q[2]);
      Eigen::Matrix<number_t, 3,1> init_euler = rot2rpy(init_quat.toRotationMatrix());
      LOG(INFO) << "init_rpy " << init_euler(0) << " " << init_euler(1) << " " << init_euler(2);
      LOG(INFO) << "init_pos " << state->_imu->pos()(0) << " " << state->_imu->pos()(1) << " " << state->_imu->pos()(2);
      LOG(INFO) << "init_vel " << state->_imu->vel()(0) << " " << state->_imu->vel()(1) << " " << state->_imu->vel()(2);
      LOG(INFO) << "init_ba " << state->_imu->bias_a()(0) << " " << state->_imu->bias_a()(1) << " " << state->_imu->bias_a()(2);
      LOG(INFO) << "init_bg " << state->_imu->bias_g()(0) << " " << state->_imu->bias_g()(1) << " " << state->_imu->bias_g()(2);
...
void VioManager::retriangulate_active_tracks(const core::CameraData &message) {
  ...
  std::map<size_t, Eigen::Matrix<number_t, 3, 3>> active_feat_linsys_A_new;
  std::map<size_t, Eigen::Matrix<number_t, 3, 1>> active_feat_linsys_b_new;
  std::map<size_t, int> active_feat_linsys_count_new;
  std::unordered_map<size_t, Eigen::Matrix<number_t, 3, 1>> active_tracks_posinG_new;
  ...
  for (auto const &cam_id : message.sensor_ids) {
    ...
    Eigen::Matrix<number_t, 3, 3> R_GtoI = state->_clones_IMU.at(active_tracks_time)->Rot();
    Eigen::Matrix<number_t, 3, 1> p_IinG = state->_clones_IMU.at(active_tracks_time)->pos();

    // Calibration for this cam_id
    Eigen::Matrix<number_t, 3, 3> R_ItoC = state->_calib_IMUtoCAM.at(cam_id)->Rot();
    Eigen::Matrix<number_t, 3, 1> p_IinC = state->_calib_IMUtoCAM.at(cam_id)->pos();

    // Convert current CAMERA position relative to global
    Eigen::Matrix<number_t, 3, 3> R_GtoCi = R_ItoC * R_GtoI;
    Eigen::Matrix<number_t, 3, 1> p_CiinG = p_IinG - R_GtoCi.transpose() * p_IinC;
    ...
    for (size_t i = 0; i < last_obs.at(cam_id).size(); i++) {
      ...
      Eigen::Matrix<number_t, 3, 1> b_i;
      b_i << pt_n.x, pt_n.y, 1;
      b_i = R_GtoCi.transpose() * b_i;
      b_i = b_i / b_i.norm();
      Eigen::Matrix<number_t, 3, 3> Bperp = skew_x(b_i);
      // Append to our linear system
      Eigen::Matrix<number_t, 3, 3> Ai = Bperp.transpose() * Bperp;
      Eigen::Matrix<number_t, 3, 1> bi = Ai * p_CiinG;
      ...
      if (active_feat_linsys_count_new.at(featid) > 3) {
        ...
        Eigen::Matrix<number_t, 3, 3> A = active_feat_linsys_A_new[featid];
        Eigen::Matrix<number_t, 3, 1> b = active_feat_linsys_b_new[featid];
        MatrixX p_FinG = A.colPivHouseholderQr().solve(b);
        MatrixX p_FinCi = R_GtoCi * (p_FinG - p_CiinG);
        // Check A and p_FinCi
        Eigen::JacobiSVD<Eigen::Matrix<number_t, 3, 3>> svd(A);
        MatrixX singularValues;
  ...
  for (const auto &feat : state->_features_SLAM) {
    Eigen::Matrix<number_t, 3, 1> p_FinG = feat.second->get_xyz(false);
    if (LandmarkRepresentation::is_relative_representation(feat.second->_feat_representation)) {
      ...
      Eigen::Matrix<number_t, 3, 3> R_ItoC = state->_calib_IMUtoCAM.at(feat.second->_anchor_cam_id)->Rot();
      Eigen::Matrix<number_t, 3, 1> p_IinC = state->_calib_IMUtoCAM.at(feat.second->_anchor_cam_id)->pos();
      // Anchor pose orientation and position
      Eigen::Matrix<number_t, 3, 3> R_GtoI = state->_clones_IMU.at(feat.second->_anchor_clone_timestamp)->Rot();
      Eigen::Matrix<number_t, 3, 1> p_IinG = state->_clones_IMU.at(feat.second->_anchor_clone_timestamp)->pos();
  ...
  Eigen::Matrix<number_t, 3, 3> R_ItoC = calibration->Rot();
  Eigen::Matrix<number_t, 3, 1> p_IinC = calibration->pos();

  // Get current IMU clone state
  std::shared_ptr<PoseJPL> clone_Ii = state->_clones_IMU.at(active_tracks_time);
  Eigen::Matrix<number_t, 3, 3> R_GtoIi = clone_Ii->Rot();
  Eigen::Matrix<number_t, 3, 1> p_IiinG = clone_Ii->pos();
  ...
  for (const auto &feat : active_tracks_posinG) {
    ...
    Eigen::Matrix<number_t, 3, 1> p_FinIi = R_GtoIi * (feat.second - p_IiinG);
    Eigen::Matrix<number_t, 3, 1> p_FinCi = R_ItoC * p_FinIi + p_IinC;
    number_t depth = p_FinCi(2);
    Eigen::Matrix<number_t, 2, 1> uv_dist;
    if (feat_uvs_in_cam0.find(feat.first) != feat_uvs_in_cam0.end()) {
      uv_dist << (number_t)feat_uvs_in_cam0.at(feat.first).x, (number_t)feat_uvs_in_cam0.at(feat.first).y;
    } else {
      Eigen::Matrix<number_t, 2, 1> uv_norm;
      uv_norm << p_FinCi(0) / depth, p_FinCi(1) / depth;
      uv_dist = state->_cam_intrinsics_cameras.at(0)->distort_d(uv_norm.cast<double>()).cast<number_t>();
    }
...
std::vector<Eigen::Matrix<number_t, 3, 1>> VioManager::get_features_SLAM() {
  std::vector<Eigen::Matrix<number_t, 3, 1>> slam_feats;
  for (auto &f : state->_features_SLAM) {
    if (type::LandmarkRepresentation::is_relative_representation(f.second->_feat_representation)) {
      // Assert that we have an anchor pose for this feature
      assert(f.second->_anchor_cam_id != -1);
      // Get calibration for our anchor camera
      Eigen::Matrix<number_t, 3, 3> R_ItoC = state->_calib_IMUtoCAM.at(f.second->_anchor_cam_id)->Rot();
      Eigen::Matrix<number_t, 3, 1> p_IinC = state->_calib_IMUtoCAM.at(f.second->_anchor_cam_id)->pos();
      // Anchor pose orientation and position
      Eigen::Matrix<number_t, 3, 3> R_GtoI = state->_clones_IMU.at(f.second->_anchor_clone_timestamp)->Rot();
      Eigen::Matrix<number_t, 3, 1> p_IinG = state->_clones_IMU.at(f.second->_anchor_clone_timestamp)->pos();
      // Feature in the global frame
