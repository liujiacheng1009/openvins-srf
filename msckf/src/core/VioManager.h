...
public:
   ...
   */
  void feed_measurement_imu(const core::ImuData &message);

  /**
   * @brief Feed function for camera measurements
   * @param message Contains our timestamp, images, and camera ids
   */
  void feed_measurement_camera(const core::CameraData &message) { track_image_and_update(message); }
  ...
  std::vector<Eigen::Matrix<number_t, 3, 1>> get_features_SLAM();

  /// Returns 3d features used in the last update in global frame
  std::vector<Eigen::Matrix<number_t, 3, 1>> get_good_features_MSCKF() { return good_features_MSCKF; }
  ...
  void get_active_tracks(double &timestamp, std::unordered_map<size_t, Eigen::Matrix<number_t, 3, 1>> &feat_posinG,
                         std::unordered_map<size_t, Eigen::Matrix<number_t, 3, 1>> &feat_tracks_uvd) {
    timestamp = active_tracks_time;
...
protected:
   ...
   * @param message Contains our timestamp, images, and camera ids
   */
  void track_image_and_update(const core::CameraData &message);

  /**
   * @brief This will do the propagation and feature updates to the state
   * @param message Contains our timestamp, images, and camera ids
   */
  void do_feature_propagate_update(const core::CameraData &message);
   ...
   */
  bool try_to_initialize(const core::CameraData &message);
   ...
   * @param message Contains our timestamp, images, and camera ids
   */
  void retriangulate_active_tracks(const core::CameraData &message);

  void preprocess_features(const core::CameraData &message, std::vector<std::shared_ptr<core::Feature>> &featsup_MSCKF, std::vector<std::shared_ptr<core::Feature>> &feats_slam_UPDATE, std::vector<std::shared_ptr<core::Feature>> &feats_slam_DELAYED);

  void remove_invalid_slam_feats(const core::CameraData &message);

  void update_cam_orents_history();
  ...
  std::shared_ptr<core::TrackBase> trackFEATS;

  /// State initializer
  std::shared_ptr<init::InertialInitializer> initializer;

  /// Boolean if we are initialized or not
  bool is_initialized_vio = false;

  // 系统重置标志
  bool should_reset_system{false};
  ...
  // zupt更新次数，为了保持输出vio pose的稳定，在3次zupt更新再输出vio pose
  int zupt_update_cnt{0};
  ...
  std::vector<Eigen::Matrix<number_t, 3, 1>> good_features_MSCKF;
  ...
  std::unordered_map<size_t, Eigen::Matrix<number_t, 3, 1>> active_tracks_posinG;
  std::unordered_map<size_t, Eigen::Matrix<number_t, 3, 1>> active_tracks_uvd;
  cv::Mat active_image;
  std::map<size_t, Eigen::Matrix<number_t, 3, 3>> active_feat_linsys_A;
  std::map<size_t, Eigen::Matrix<number_t, 3, 1>> active_feat_linsys_b;
  std::map<size_t, int> active_feat_linsys_count;

  constexpr static float kGoodParallaxThreshold = 5.;
  std::vector<Eigen::Matrix<number_t, 3, 3>> cam_orents_history;

  constexpr static int kMaxInitSlamFeatNum = 10;
};
