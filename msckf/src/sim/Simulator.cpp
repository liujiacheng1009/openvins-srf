...
Simulator::Simulator(VioManagerOptions &params_) {
  ...
  for (auto const &tmp : params_.camera_intrinsics) {
    auto tmp_cast = std::dynamic_pointer_cast<core::CamEqui>(tmp.second);
    if (tmp_cast != nullptr) {
      params.camera_intrinsics.insert({tmp.first, std::make_shared<core::CamEqui>(tmp.second->w(), tmp.second->h())});
      params.camera_intrinsics.at(tmp.first)->set_value(params_.camera_intrinsics.at(tmp.first)->get_value());
    } else {
      params.camera_intrinsics.insert({tmp.first, std::make_shared<core::CamRadtan>(tmp.second->w(), tmp.second->h())});
      params.camera_intrinsics.at(tmp.first)->set_value(params_.camera_intrinsics.at(tmp.first)->get_value());
    }
  }
  ...
  spline = std::make_shared<core::BsplineSE3>();
...
void Simulator::perturb_parameters(std::mt19937 gen_state, VioManagerOptions &params_) {
  ...
  // One std generator
  std::normal_distribution<number_t> w(0, 1);
  ...
  for (int i = 0; i < params_.state_options.num_cameras; i++) {
    ...
    MatrixX intrinsics = params_.camera_intrinsics.at(i)->get_value();
    for (int r = 0; r < 4; r++) {
    ...
    Eigen::Matrix<number_t, 3, 1> w_vec;
...
bool Simulator::get_next_imu(double &time_imu, Eigen::Matrix<number_t, 3, 1> &wm, Eigen::Matrix<number_t, 3, 1> &am)
{
  ...
  Eigen::Matrix<number_t, 3, 3> Dw = State::Dm(params.state_options.imu_model, params.vec_dw);
  Eigen::Matrix<number_t, 3, 3> Da = State::Dm(params.state_options.imu_model, params.vec_da);
  Eigen::Matrix<number_t, 3, 3> Tg = State::Tg(params.vec_tg);

  // Get the readings with the imu intrinsic "distortion"
  Eigen::Matrix3d Tw = Dw.cast<double>().colPivHouseholderQr().solve(Eigen::Matrix3d::Identity());
  Eigen::Matrix3d Ta = Da.cast<double>().colPivHouseholderQr().solve(Eigen::Matrix3d::Identity());
  Eigen::Vector3d omega_inGYRO = Tw * quat_2_Rot(params.q_GYROtoIMU.cast<double>()).transpose() * omega_inI + Tg.cast<double>() * accel_inI;
  Eigen::Vector3d accel_inACC = Ta * quat_2_Rot(params.q_ACCtoIMU.cast<double>()).transpose() * accel_inI;
...
std::vector<std::pair<size_t, Eigen::VectorXf>> Simulator::project_pointcloud(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG,
  ...
  Eigen::Matrix<double, 3, 3> R_ItoC = quat_2_Rot(params.camera_extrinsics.at(camid).block(0, 0, 4, 1)).cast<double>();
  Eigen::Matrix<double, 3, 1> p_IinC = params.camera_extrinsics.at(camid).block(4, 0, 3, 1).cast<double>();
  std::shared_ptr<core::CamBase> camera = params.camera_intrinsics.at(camid);
