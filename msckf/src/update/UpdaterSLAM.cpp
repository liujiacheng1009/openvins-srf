...
UpdaterSLAM::UpdaterSLAM(UpdaterOptions &options_slam, core::FeatureInitializerOptions &feat_init_options)
  ...
  initializer_feat = std::shared_ptr<core::FeatureInitializer>(new core::FeatureInitializer(feat_init_options));
...
void UpdaterSLAM::delayed_init(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>> &feature_vec) {
  ...
  for (const auto &clone_calib : state->_calib_IMUtoCAM) {
    ...
    for (const auto &clone_imu : state->_clones_IMU) {
      ...
      Eigen::Matrix<number_t, 3, 3> R_GtoCi = clone_calib.second->Rot() * clone_imu.second->Rot();
      Eigen::Matrix<number_t, 3, 1> p_CioinG = clone_imu.second->pos() - R_GtoCi.transpose() * clone_calib.second->pos();
  ...
  while (it2 != feature_vec.end()) {
    ...
    MatrixX R = sigma_pix_sq * MatrixX::Identity(res.rows(), res.rows());

    // Try to initialize, delete new pointer if we failed
    double chi2_multipler = _options_slam.chi2_multipler;
    if (FilterWrapper::initialize(state, landmark, Hx_order, H_x, H_f, R, res, chi2_multipler)) {
...
void UpdaterSLAM::update(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>> &feature_vec) {
  ...
  VectorX res_big = VectorX::Zero(max_meas_size);
  MatrixX Hx_big = MatrixX::Zero(max_meas_size, max_hx_size);
  MatrixX R_big = MatrixX::Identity(max_meas_size, max_meas_size);
  ...
  while (it2 != feature_vec.end()) {
    ...
    // Chi2 distance check
#ifdef VIO_USE_SRIF
    MatrixX S = FilterWrapper::get_HxPmargHxT(state, Hx_order, H_x);
#else
    MatrixX S = FilterWrapper::get_HxPmargHxT(state, Hxf_order, H_xf);
#endif
    double sigma_pix_sq = _options_slam.sigma_pix_sq;
    S.diagonal() += sigma_pix_sq * VectorX::Ones(S.rows());
  ...
  FilterWrapper::Update(state, Hx_order_big, Hx_big, res_big, R_big, GlobalFlagPool::getJointUpdate());
}
...
void UpdaterSLAM::change_anchors(std::shared_ptr<State> state) {
  ...
  if (GlobalFlagPool::getJointAnchorChange())
    FilterWrapper::JointPropagation(state);
}
...
void UpdaterSLAM::perform_anchor_change(std::shared_ptr<State> state, std::shared_ptr<Landmark> landmark, double new_anchor_timestamp,
  ...
  Eigen::Matrix<number_t, 3, 3> R_GtoIOLD = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->Rot();
  Eigen::Matrix<number_t, 3, 3> R_GtoOLD = state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->Rot() * R_GtoIOLD;
  Eigen::Matrix<number_t, 3, 1> p_OLDinG = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->pos() -
                                         R_GtoOLD.transpose() * state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->pos();

  // NEW: anchor camera position and orientation
  Eigen::Matrix<number_t, 3, 3> R_GtoINEW = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->Rot();
  Eigen::Matrix<number_t, 3, 3> R_GtoNEW = state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->Rot() * R_GtoINEW;
  Eigen::Matrix<number_t, 3, 1> p_NEWinG = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->pos() -
  ...
  Eigen::Matrix<number_t, 3, 3> R_OLDtoNEW = R_GtoNEW * R_GtoOLD.transpose();
  Eigen::Matrix<number_t, 3, 1> p_OLDinNEW = R_GtoNEW * (p_OLDinG - p_NEWinG);
  ...
  // OLD: anchor camera position and orientation
  Eigen::Matrix<number_t, 3, 3> R_GtoIOLD_fej = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->Rot_fej();
  Eigen::Matrix<number_t, 3, 3> R_GtoOLD_fej = state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->Rot() * R_GtoIOLD_fej;
  Eigen::Matrix<number_t, 3, 1> p_OLDinG_fej = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->pos_fej() -
                                             R_GtoOLD_fej.transpose() * state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->pos();

  // NEW: anchor camera position and orientation
  Eigen::Matrix<number_t, 3, 3> R_GtoINEW_fej = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->Rot_fej();
  Eigen::Matrix<number_t, 3, 3> R_GtoNEW_fej = state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->Rot() * R_GtoINEW_fej;
  Eigen::Matrix<number_t, 3, 1> p_NEWinG_fej = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->pos_fej() -
                                             R_GtoNEW_fej.transpose() * state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->pos();

  // Calculate transform between the old anchor and new one
  Eigen::Matrix<number_t, 3, 3> R_OLDtoNEW_fej = R_GtoNEW_fej * R_GtoOLD_fej.transpose();
  Eigen::Matrix<number_t, 3, 1> p_OLDinNEW_fej = R_GtoNEW_fej * (p_OLDinG_fej - p_NEWinG_fej);
  new_feat.p_FinA_fej = R_OLDtoNEW_fej * landmark->get_xyz(true) + p_OLDinNEW_fej;

  // Get Jacobians of p_FinG wrt new representation
  MatrixX H_f_new;
  std::vector<MatrixX> H_x_new;
  ...
  MatrixX Phi = MatrixX::Zero(phisize, current_it);
  MatrixX Q = MatrixX::Zero(phisize, phisize);
  ...
  if (phisize == 1) {
    ...
    H_f_new_inv = H_f_new.colPivHouseholderQr().solve(Eigen::Matrix<number_t, 3, 3>::Identity());
  }
...
#else
  assert(phisize == 3);
  MatrixX Phi_lm_inv = (H_f_new_inv * H_f_old).colPivHouseholderQr().solve(Eigen::Matrix<number_t, 3, 3>::Identity());
  // Place Jacobians for old anchor
  for (size_t i = 0; i < H_x_old.size(); i++)
  {
    Phi.block(0, Phi_id_map.at(x_order_old[i]), phisize, x_order_old[i]->size()).noalias() -= Phi_lm_inv * H_f_new_inv * H_x_old[i];
  }

  // Place Jacobians for old feat
  Phi.block(0, Phi_id_map.at(landmark), phisize, phisize) = Phi_lm_inv;

  // Place Jacobians for new anchor
  for (size_t i = 0; i < H_x_new.size(); i++)
  {
    Phi.block(0, Phi_id_map.at(x_order_new[i]), phisize, x_order_new[i]->size()).noalias() += Phi_lm_inv * H_f_new_inv * H_x_new[i];
  }
#endif
  // Perform covariance propagation
  FilterWrapper::Propagation(state, phi_order_NEW, phi_order_OLD, Phi, Q, GlobalFlagPool::getJointAnchorChange());
