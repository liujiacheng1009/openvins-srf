...
void UpdaterHelper::get_feature_jacobian_representation(std::shared_ptr<State> state, UpdaterHelperFeature &feature, MatrixX &H_f,
                                                        std::vector<MatrixX> &H_x, std::vector<std::shared_ptr<Type>> &x_order) {
  ...
  if (feature.feat_representation == LandmarkRepresentation::Representation::GLOBAL_FULL_INVERSE_DEPTH) {
    ...
    Eigen::Matrix<number_t, 3, 1> p_FinG = (state->_options.do_fej) ? feature.p_FinG_fej : feature.p_FinG;
  ...
  Eigen::Matrix<number_t, 3, 3> R_ItoC = state->_calib_IMUtoCAM.at(feature.anchor_cam_id)->Rot();
  Eigen::Matrix<number_t, 3, 1> p_IinC = state->_calib_IMUtoCAM.at(feature.anchor_cam_id)->pos();
  Eigen::Matrix<number_t, 3, 3> R_GtoI = state->_clones_IMU.at(feature.anchor_clone_timestamp)->Rot();
  Eigen::Matrix<number_t, 3, 1> p_IinG = state->_clones_IMU.at(feature.anchor_clone_timestamp)->pos();
  Eigen::Matrix<number_t, 3, 1> p_FinA = feature.p_FinA;
  ...
  if (state->_options.do_fej) {
    ...
    Eigen::Matrix<number_t, 3, 1> p_FinG_best = R_GtoI.transpose() * R_ItoC.transpose() * (feature.p_FinA - p_IinC) + p_IinG;
  ...
  Eigen::Matrix<number_t, 3, 3> R_CtoG = R_GtoI.transpose() * R_ItoC.transpose();

  // Jacobian for our anchor pose
  Eigen::Matrix<number_t, 3, 6> H_anc;
  ...
  if (state->_options.do_calib_camera_pose) {
    Eigen::Matrix<number_t, 3, 6> H_calib;
  ...
  if (feature.feat_representation == LandmarkRepresentation::Representation::ANCHORED_FULL_INVERSE_DEPTH) {
    ...
    d_pfinA_dpinv << -(1.0 / rho) * sin_th * sin_phi, (1.0 / rho) * cos_th * cos_phi, -(1.0 / (rho * rho)) * cos_th * sin_phi,
        (1.0 / rho) * cos_th * sin_phi, (1.0 / rho) * sin_th * cos_phi, -(1.0 / (rho * rho)) * sin_th * sin_phi, 0.0,
        -(1.0 / rho) * sin_phi, -(1.0 / (rho * rho)) * cos_phi;
    H_f = R_CtoG * d_pfinA_dpinv.cast<number_t>();
    return;
  }
  ...
  if (feature.feat_representation == LandmarkRepresentation::Representation::ANCHORED_INVERSE_DEPTH_SINGLE) {
    ...
    Eigen::Matrix<number_t, 3, 1> bearing = rho * p_FinA;

    // Jacobian of anchored 3D position wrt inverse depth parameters
    Eigen::Matrix<number_t, 3, 1> d_pfinA_drho;
...
void UpdaterHelper::get_feature_jacobian_full(std::shared_ptr<State> state, UpdaterHelperFeature &feature, MatrixX &H_f,
                                              MatrixX &H_x, VectorX &res, std::vector<std::shared_ptr<Type>> &x_order) {
  ...
  Eigen::Matrix<number_t, 3, 1> p_FinG = feature.p_FinG;
  if (LandmarkRepresentation::is_relative_representation(feature.feat_representation)) {
    // Assert that we have an anchor pose for this feature
    assert(feature.anchor_cam_id != -1);
  ...
  Eigen::Matrix<number_t, 3, 1> p_FinG_fej = feature.p_FinG_fej;
  if (LandmarkRepresentation::is_relative_representation(feature.feat_representation)) {
    p_FinG_fej = p_FinG;
  }
  ...
  res = VectorX::Zero(2 * total_meas);
  H_f = MatrixX::Zero(2 * total_meas, jacobsize);
  H_x = MatrixX::Zero(2 * total_meas, total_hx);
  ...
  MatrixX dpfg_dlambda;
  std::vector<MatrixX> dpfg_dx;
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      Eigen::Matrix<number_t, 3, 1> p_FinIi = R_GtoIi * (p_FinG - p_IiinG);
      ...
      res.block(2 * c, 0, 2, 1) = (uv_m - uv_dist).cast<number_t>();
     ...
     MatrixX dz_dzn, dz_dzeta;
      ...
      MatrixX dpfc_dpfg = R_ItoC * R_GtoIi;

      // Derivative of p_FinCi in respect to camera clone state
      MatrixX dpfc_dclone = MatrixX::Zero(3, 6);
      ...
      MatrixX dz_dpfc = dz_dzn * dzn_dpfc;
      MatrixX dz_dpfg = dz_dpfc * dpfc_dpfg;
      ...
      if (state->_options.do_calib_camera_pose) {
        ...
        MatrixX dpfc_dcalib = MatrixX::Zero(3, 6);
        dpfc_dcalib.block(0, 0, 3, 3) = skew_x(p_FinCi - p_IinC);
        dpfc_dcalib.block(0, 3, 3, 3) = Eigen::Matrix<number_t, 3, 3>::Identity();
...
void UpdaterHelper::nullspace_project_inplace(MatrixX &H_f, MatrixX &H_x, VectorX &res) {
...
void UpdaterHelper::measurement_compress_inplace(MatrixX &H_x, VectorX &res) {
