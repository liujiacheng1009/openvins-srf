...
public:
  ...
  static void StackMeasures(std::shared_ptr<State> state, const std::vector<std::shared_ptr<type::Type>> &H_order, const MatrixX &H,
                            const VectorX &res, const MatrixX &R);

  static void Update(std::shared_ptr<State> state, const MatrixX &FT, const MatrixX &H_big,
  ...
  static MatrixX get_marginal_sqrtcov(std::shared_ptr<State> state,
                                              const std::vector<std::shared_ptr<type::Type>> &small_variables);
  /**
   ...
   * Please use the other interface functions in the FilterSRF to progamatically change to covariance.
   *
   * @param state Pointer to state
