...
State::State(StateOptions &options) {
  ...
  if (options.imu_model == StateOptions::ImuModel::KALIBR) {
    ...
    Eigen::Matrix<number_t, 6, 1> _imu_default = Eigen::Matrix<number_t, 6, 1>::Zero();
  ...
  _Cov = std::pow(1e-3, 2) * MatrixX::Identity(current_id, current_id);

  // Finally, set some of our priors for our calibration parameters
  if (_options.do_calib_imu_intrinsics) {
    _Cov.block(_calib_imu_dw->id(), _calib_imu_dw->id(), 6, 6) = std::pow(0.005, 2) * Eigen::Matrix<number_t, 6, 6>::Identity();
    _Cov.block(_calib_imu_da->id(), _calib_imu_da->id(), 6, 6) = std::pow(0.008, 2) * Eigen::Matrix<number_t, 6, 6>::Identity();
    if (_options.do_calib_imu_g_sensitivity) {
      _Cov.block(_calib_imu_tg->id(), _calib_imu_tg->id(), 9, 9) = std::pow(0.005, 2) * Eigen::Matrix<number_t, 9, 9>::Identity();
    }
    if (_options.imu_model == StateOptions::ImuModel::KALIBR) {
      _Cov.block(_calib_imu_GYROtoIMU->id(), _calib_imu_GYROtoIMU->id(), 3, 3) = std::pow(0.005, 2) * Eigen::Matrix<number_t, 3,3>::Identity();
    } else {
      _Cov.block(_calib_imu_ACCtoIMU->id(), _calib_imu_ACCtoIMU->id(), 3, 3) = std::pow(0.005, 2) * Eigen::Matrix<number_t, 3, 3>::Identity();
    }
  }
  if (_options.do_calib_camera_timeoffset) {
  ...
  if (_options.do_calib_camera_pose) {
    for (int i = 0; i < _options.num_cameras; i++) {
      _Cov.block(_calib_IMUtoCAM.at(i)->id(), _calib_IMUtoCAM.at(i)->id(), 3, 3) = std::pow(0.005, 2) * MatrixX::Identity(3, 3);
      _Cov.block(_calib_IMUtoCAM.at(i)->id() + 3, _calib_IMUtoCAM.at(i)->id() + 3, 3, 3) =
          std::pow(0.015, 2) * MatrixX::Identity(3, 3);
    }
  }
  if (_options.do_calib_camera_intrinsics) {
    for (int i = 0; i < _options.num_cameras; i++) {
      _Cov.block(_cam_intrinsics.at(i)->id(), _cam_intrinsics.at(i)->id(), 4, 4) = std::pow(1.0, 2) * MatrixX::Identity(4, 4);
      _Cov.block(_cam_intrinsics.at(i)->id() + 4, _cam_intrinsics.at(i)->id() + 4, 4, 4) =
          std::pow(0.005, 2) * MatrixX::Identity(4, 4);
    }
  }
}
