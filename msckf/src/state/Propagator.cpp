...
void Propagator::propagate_and_clone(std::shared_ptr<State> state, double timestamp) {
  ...
  std::vector<core::ImuData> prop_data;
  {
  ...
  MatrixX Phi_summed = MatrixX::Identity(state->imu_intrinsic_size() + 15, state->imu_intrinsic_size() + 15);
  MatrixX Qd_summed = MatrixX::Zero(state->imu_intrinsic_size() + 15, state->imu_intrinsic_size() + 15);
  double dt_summed = 0;
  ...
  Eigen::Matrix<number_t, 3, 1> last_a = Eigen::Matrix<number_t, 3, 1>::Zero();
  Eigen::Matrix<number_t, 3, 1> last_w = Eigen::Matrix<number_t, 3, 1>::Zero();
  if (!prop_data.empty())
  {
  ...
  FilterWrapper::PropagationAndClone(state, Phi_order, Phi_order, Phi_summed, Qd_summed, last_w);
}

bool Propagator::fast_state_propagate(std::shared_ptr<State> state, double timestamp, Eigen::Matrix<number_t, 13, 1> &state_plus,
                                      Eigen::Matrix<number_t, 12, 12> &covariance) {
  ...
  if (!cache_imu_valid) {
    ...
    cache_state_covariance = FilterWrapper::get_marginal_covariance(state, {state->_imu});
  ...
  // Biases
  Eigen::Matrix<number_t, 3, 1> bias_g = cache_state_est.block(10, 0, 3, 1);
  Eigen::Matrix<number_t, 3, 1> bias_a = cache_state_est.block(13, 0, 3, 1);

  // IMU intrinsic calibration estimates (static)
  Eigen::Matrix<number_t, 3, 3> Dw = State::Dm(state->_options.imu_model, state->_calib_imu_dw->value());
  Eigen::Matrix<number_t, 3, 3> Da = State::Dm(state->_options.imu_model, state->_calib_imu_da->value());
  Eigen::Matrix<number_t, 3, 3> Tg = State::Tg(state->_calib_imu_tg->value());
  Eigen::Matrix<number_t, 3, 3> R_ACCtoIMU = state->_calib_imu_ACCtoIMU->Rot();
  Eigen::Matrix<number_t, 3, 3> R_GYROtoIMU = state->_calib_imu_GYROtoIMU->Rot();
  ...
  for (size_t i = 0; i < prop_data.size() - 1; i++) {
    ...
    number_t dt = static_cast<number_t>(data_plus.timestamp - data_minus.timestamp);

    // Corrected imu acc measurements with our current biases
    Eigen::Matrix<number_t, 3, 1> a_hat1 = R_ACCtoIMU * Da * (data_minus.am - bias_a);
    Eigen::Matrix<number_t, 3, 1> a_hat2 = R_ACCtoIMU * Da * (data_plus.am - bias_a);
    Eigen::Matrix<number_t, 3, 1> a_hat = 0.5 * (a_hat1 + a_hat2);

    // Corrected imu gyro measurements with our current biases
    Eigen::Matrix<number_t, 3, 1> w_hat1 = R_GYROtoIMU * Dw * (data_minus.wm - bias_g - Tg * a_hat1);
    Eigen::Matrix<number_t, 3, 1> w_hat2 = R_GYROtoIMU * Dw * (data_plus.wm - bias_g - Tg * a_hat2);
    Eigen::Matrix<number_t, 3, 1> w_hat = 0.5 * (w_hat1 + w_hat2);

    // Current state estimates
    Eigen::Matrix<number_t, 3, 3> R_Gtoi = quat_2_Rot(cache_state_est.block(0, 0, 4, 1));
    Eigen::Matrix<number_t, 3, 1> v_iinG = cache_state_est.block(7, 0, 3, 1);
    Eigen::Matrix<number_t, 3, 1> p_iinG = cache_state_est.block(4, 0, 3, 1);
    ...
    Eigen::Matrix<number_t, 15, 15> F = Eigen::Matrix<number_t, 15, 15>::Zero();
    ...
    F.block(3, 6, 3, 3) = Eigen::Matrix<number_t,3,3>::Identity() * dt;
    F.block(3, 12, 3, 3) = -0.5 * R_Gtoi.transpose() * dt * dt;
    F.block(3, 3, 3, 3).setIdentity();
    Eigen::Matrix<number_t, 15, 12> G = Eigen::Matrix<number_t, 15, 12>::Zero();
    ...
    Eigen::Matrix<number_t, 15, 15> Qd = Eigen::Matrix<number_t, 15, 15>::Zero();
    Eigen::Matrix<number_t, 12, 12> Qc = Eigen::Matrix<number_t, 12, 12>::Zero();
    Qc.block(0, 0, 3, 3) = _noises.sigma_w_2 / dt * Eigen::Matrix<number_t,3,3>::Identity();
    Qc.block(3, 3, 3, 3) = _noises.sigma_a_2 / dt * Eigen::Matrix<number_t,3,3>::Identity();
    Qc.block(6, 6, 3, 3) = _noises.sigma_wb_2 * dt * Eigen::Matrix<number_t,3,3>::Identity();
    Qc.block(9, 9, 3, 3) = _noises.sigma_ab_2 * dt * Eigen::Matrix<number_t,3,3>::Identity();
    Qd = G * Qc * G.transpose();
  ...
  Eigen::Matrix<number_t, 4, 1> q_Gtoi = cache_state_est.block(0, 0, 4, 1);
  Eigen::Matrix<number_t, 3, 1> v_iinG = cache_state_est.block(7, 0, 3, 1);
  Eigen::Matrix<number_t, 3, 1> p_iinG = cache_state_est.block(4, 0, 3, 1);
  state_plus.setZero();
  ...
  Eigen::Matrix<number_t, 3, 1> last_a = R_ACCtoIMU * Da * (prop_data.at(prop_data.size() - 1).am - bias_a);
  Eigen::Matrix<number_t, 3, 1> last_w = R_GYROtoIMU * Dw * (prop_data.at(prop_data.size() - 1).wm - bias_g - Tg * last_a);
  ...
  covariance.setZero();
  Eigen::Matrix<number_t, 15, 15> Phi = Eigen::Matrix<number_t, 15, 15>::Identity();
  Phi.block(6, 6, 3, 3) = quat_2_Rot(q_Gtoi);
  MatrixX covariance_tmp = Phi * cache_state_covariance * Phi.transpose();
  covariance.block(0, 0, 9, 9) = covariance_tmp.block(0, 0, 9, 9);
  number_t dt = static_cast<number_t>(prop_data.at(prop_data.size() - 1).timestamp - prop_data.at(prop_data.size() - 2).timestamp);
  covariance.block(9, 9, 3, 3) = _noises.sigma_w_2 / dt * Eigen::Matrix<number_t, 3, 3>::Identity();
  return true;
}

std::vector<core::ImuData> Propagator::select_imu_readings(const std::vector<core::ImuData> &imu_data, double time0, double time1,
                                                              bool warn) {
  ...
  for (size_t i = 0; i < imu_data.size() - 1; i++) {
    ...
    if (imu_data.at(i + 1).timestamp > time0 && imu_data.at(i).timestamp < time0) {
      core::ImuData data = Propagator::interpolate_data(imu_data.at(i), imu_data.at(i + 1), time0);
    ...
    if (imu_data.at(i + 1).timestamp > time1) {
      ...
      if (imu_data.at(i).timestamp > time1 && i == 0) {
        ...
        core::ImuData data = interpolate_data(imu_data.at(i - 1), imu_data.at(i), time1);
        prop_data.push_back(data);
      ...
      if (prop_data.at(prop_data.size() - 1).timestamp != time1) {
        core::ImuData data = interpolate_data(imu_data.at(i), imu_data.at(i + 1), time1);
        prop_data.push_back(data);
  ...
  if (prop_data.at(prop_data.size() - 1).timestamp != time1) {
    ...
    core::ImuData data = interpolate_data(imu_data.at(imu_data.size() - 2), imu_data.at(imu_data.size() - 1), time1);
    prop_data.push_back(data);
...
void Propagator::predict_and_compute(std::shared_ptr<State> state, const core::ImuData &data_minus, const core::ImuData &data_plus,
                                     MatrixX &F, MatrixX &Qd) {
  ...
  // Corrected imu acc measurements with our current biases
  Eigen::Matrix<number_t, 3, 1> a_hat1 = data_minus.am - state->_imu->bias_a();
  Eigen::Matrix<number_t, 3, 1> a_hat2 = data_plus.am - state->_imu->bias_a();
  Eigen::Matrix<number_t, 3, 1> a_hat_avg = .5 * (a_hat1 + a_hat2);

  // Convert "raw" imu to its corrected frame using the IMU intrinsics
  Eigen::Matrix<number_t, 3, 1> a_uncorrected = a_hat_avg;
  Eigen::Matrix<number_t, 3, 3> R_ACCtoIMU = state->_calib_imu_ACCtoIMU->Rot();
  ...
  Eigen::Matrix<number_t, 3, 1> w_hat1 = data_minus.wm - state->_imu->bias_g() - Tg * a_hat1;
  Eigen::Matrix<number_t, 3, 1> w_hat2 = data_plus.wm - state->_imu->bias_g() - Tg * a_hat2;
  Eigen::Matrix<number_t, 3, 1> w_hat_avg = .5 * (w_hat1 + w_hat2);

  // Convert "raw" imu to its corrected frame using the IMU intrinsics
  Eigen::Matrix<number_t, 3, 1> w_uncorrected = w_hat_avg;
  Eigen::Matrix<number_t, 3, 3> R_GYROtoIMU = state->_calib_imu_GYROtoIMU->Rot();
  ...
  Eigen::Matrix<number_t, 3, 18> Xi_sum = Eigen::Matrix<number_t, 3, 18>::Zero(3, 18);
  ...
  Eigen::Matrix<number_t, 4, 1> new_q;
  Eigen::Matrix<number_t, 3, 1> new_v, new_p;
  ...
  F = MatrixX::Zero(state->imu_intrinsic_size() + 15, state->imu_intrinsic_size() + 15);
  MatrixX G = MatrixX::Zero(state->imu_intrinsic_size() + 15, 12);
  if (state->_options.integration_method == StateOptions::IntegrationMethod::RK4 ||
      state->_options.integration_method == StateOptions::IntegrationMethod::ANALYTICAL) {
  ...
  // Equations (129) amd (130) of Trawny tech report
  Eigen::Matrix<number_t, 12, 12> Qc = Eigen::Matrix<number_t, 12, 12>::Zero();
  Qc.block(0, 0, 3, 3) = std::pow(_noises.sigma_w, 2) / dt * Eigen::Matrix<number_t,3,3>::Identity();
  Qc.block(3, 3, 3, 3) = std::pow(_noises.sigma_a, 2) / dt * Eigen::Matrix<number_t,3,3>::Identity();
  Qc.block(6, 6, 3, 3) = std::pow(_noises.sigma_wb, 2) / dt * Eigen::Matrix<number_t,3,3>::Identity();
  Qc.block(9, 9, 3, 3) = std::pow(_noises.sigma_ab, 2) / dt * Eigen::Matrix<number_t,3,3>::Identity();

  // Compute the noise injected into the state over the interval
  Qd = MatrixX::Zero(state->imu_intrinsic_size() + 15, state->imu_intrinsic_size() + 15);
  Qd = G * Qc * G.transpose();
  Qd = 0.5 * (Qd + Qd.transpose());

  // Now replace imu estimate and fej with propagated values
  Eigen::Matrix<number_t, 16, 1> imu_x = state->_imu->value();
...
void Propagator::predict_mean_discrete(std::shared_ptr<State> state, number_t dt, const Eigen::Matrix<number_t,3,1> &w_hat, const Eigen::Matrix<number_t,3,1> &a_hat,
                                       Eigen::Matrix<number_t, 4,1> &new_q, Eigen::Matrix<number_t,3,1> &new_v, Eigen::Matrix<number_t,3,1> &new_p) {

  // Pre-compute things
  number_t w_norm = w_hat.norm();
  Eigen::Matrix<number_t, 4,4> I_4x4 = Eigen::Matrix<number_t, 4,4>::Identity();
  Eigen::Matrix<number_t,3,3> R_Gtoi = state->_imu->Rot();

  // Orientation: Equation (101) and (103) and of Trawny indirect TR
  Eigen::Matrix<number_t, 4, 4> bigO;
  if (w_norm > 1e-12) {
...
void Propagator::predict_mean_rk4(std::shared_ptr<State> state, number_t dt, const Eigen::Matrix<number_t,3,1> &w_hat1, const Eigen::Matrix<number_t,3,1> &a_hat1,
                                  const Eigen::Matrix<number_t,3,1> &w_hat2, const Eigen::Matrix<number_t,3,1> &a_hat2, Eigen::Matrix<number_t, 4,1> &new_q,
                                  Eigen::Matrix<number_t,3,1> &new_v, Eigen::Matrix<number_t,3,1> &new_p) {

  // Pre-compute things
  Eigen::Matrix<number_t, 3, 1> w_hat = w_hat1;
  Eigen::Matrix<number_t, 3, 1> a_hat = a_hat1;
  Eigen::Matrix<number_t, 3, 1> w_alpha = (w_hat2 - w_hat1) / dt;
  Eigen::Matrix<number_t, 3, 1> a_jerk = (a_hat2 - a_hat1) / dt;

  // y0 ================
  Eigen::Matrix<number_t, 4, 1> q_0 = state->_imu->quat();
  Eigen::Matrix<number_t, 3, 1> p_0 = state->_imu->pos();
  Eigen::Matrix<number_t, 3, 1> v_0 = state->_imu->vel();
  // k1 ================
  Eigen::Matrix<number_t, 4, 1> dq_0 = {0, 0, 0, 1};
  Eigen::Matrix<number_t, 4, 1> q0_dot = 0.5 * Omega(w_hat) * dq_0;
  Eigen::Matrix<number_t, 3, 1> p0_dot = v_0;
  Eigen::Matrix<number_t, 3, 3> R_Gto0 = quat_2_Rot(quat_multiply(dq_0, q_0));
  Eigen::Matrix<number_t, 3, 1> v0_dot = R_Gto0.transpose() * a_hat - _gravity;
  Eigen::Matrix<number_t, 4, 1> k1_q = q0_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k1_p = p0_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k1_v = v0_dot * dt;

  // k2 ================
  w_hat += 0.5 * w_alpha * dt;
  a_hat += 0.5 * a_jerk * dt;

  Eigen::Matrix<number_t, 4, 1> dq_1 = quatnorm(dq_0 + 0.5 * k1_q);
  // Eigen::Matrix<number_t,3,1> p_1 = p_0+0.5*k1_p;
  Eigen::Matrix<number_t, 3, 1> v_1 = v_0 + 0.5 * k1_v;

  Eigen::Matrix<number_t, 4, 1> q1_dot = 0.5 * Omega(w_hat) * dq_1;
  Eigen::Matrix<number_t, 3, 1> p1_dot = v_1;
  Eigen::Matrix<number_t, 3, 3> R_Gto1 = quat_2_Rot(quat_multiply(dq_1, q_0));
  Eigen::Matrix<number_t, 3, 1> v1_dot = R_Gto1.transpose() * a_hat - _gravity;
  Eigen::Matrix<number_t, 4, 1> k2_q = q1_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k2_p = p1_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k2_v = v1_dot * dt;

  // k3 ================
  Eigen::Matrix<number_t, 4, 1> dq_2 = quatnorm(dq_0 + 0.5 * k2_q);
  // Eigen::Matrix<number_t,3,1> p_2 = p_0+0.5*k2_p;
  Eigen::Matrix<number_t, 3, 1> v_2 = v_0 + 0.5 * k2_v;

  Eigen::Matrix<number_t, 4, 1> q2_dot = 0.5 * Omega(w_hat) * dq_2;
  Eigen::Matrix<number_t, 3, 1> p2_dot = v_2;
  Eigen::Matrix<number_t, 3, 3> R_Gto2 = quat_2_Rot(quat_multiply(dq_2, q_0));
  Eigen::Matrix<number_t, 3, 1> v2_dot = R_Gto2.transpose() * a_hat - _gravity;
  Eigen::Matrix<number_t, 4, 1> k3_q = q2_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k3_p = p2_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k3_v = v2_dot * dt;

  // k4 ================
  w_hat += 0.5 * w_alpha * dt;
  a_hat += 0.5 * a_jerk * dt;

  Eigen::Matrix<number_t, 4, 1> dq_3 = quatnorm(dq_0 + k3_q);
  // Eigen::Matrix<number_t,3,1> p_3 = p_0+k3_p;
  Eigen::Matrix<number_t, 3, 1> v_3 = v_0 + k3_v;
  Eigen::Matrix<number_t, 4, 1> q3_dot = 0.5 * Omega(w_hat) * dq_3;
  Eigen::Matrix<number_t, 3, 1> p3_dot = v_3;
  Eigen::Matrix<number_t, 3, 3> R_Gto3 = quat_2_Rot(quat_multiply(dq_3, q_0));
  Eigen::Matrix<number_t, 3, 1> v3_dot = R_Gto3.transpose() * a_hat - _gravity;
  Eigen::Matrix<number_t, 4, 1> k4_q = q3_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k4_p = p3_dot * dt;
  Eigen::Matrix<number_t, 3, 1> k4_v = v3_dot * dt;

  // y+dt ================
  Eigen::Matrix<number_t, 4, 1> dq = quatnorm(dq_0 + (1.0 / 6.0) * k1_q + (1.0 / 3.0) * k2_q + (1.0 / 3.0) * k3_q + (1.0 / 6.0) * k4_q);
...
void Propagator::compute_Xi_sum(std::shared_ptr<State> state, double dt, const Eigen::Matrix<number_t, 3, 1> &w_hat, const Eigen::Matrix<number_t, 3, 1> &a_hat,
                                Eigen::Matrix<number_t, 3, 18> &Xi_sum)
{

  // Decompose our angular velocity into a direction and amount
  number_t w_norm = w_hat.norm();
  double d_th = w_norm * dt;
  Eigen::Matrix<number_t, 3, 1> k_hat = Eigen::Matrix<number_t, 3, 1>::Zero();
  if (w_norm > 1e-12)
  {
    k_hat = w_hat / w_norm;
  }
  ...
  Eigen::Matrix<double, 3, 3> I_3x3 = Eigen::Matrix<double, 3, 3>::Identity();
  ...
  Eigen::Matrix<double, 3, 3> sK = core::skew_x(k_hat.cast<double>());
  Eigen::Matrix<double, 3, 3> sK2 = sK * sK;
  Eigen::Matrix<double, 3, 3> sA = core::skew_x(a_hat.cast<double>());

  // Integration components will be used later
  Eigen::Matrix<double, 3, 3> R_ktok1, Xi_1, Xi_2, Jr_ktok1, Xi_3, Xi_4;
  R_ktok1 = core::exp_so3(-w_hat.cast<double>() * dt);
  Jr_ktok1 = core::Jr_so3(-w_hat.cast<double>() * dt);
  ...
  Xi_sum.setZero();
  Xi_sum.block(0, 0, 3, 3) = R_ktok1.cast<number_t>();
  Xi_sum.block(0, 3, 3, 3) = Xi_1.cast<number_t>();
  Xi_sum.block(0, 6, 3, 3) = Xi_2.cast<number_t>();
  Xi_sum.block(0, 9, 3, 3) = Jr_ktok1.cast<number_t>();
  Xi_sum.block(0, 12, 3, 3) = Xi_3.cast<number_t>();
  Xi_sum.block(0, 15, 3, 3) = Xi_4.cast<number_t>();
}

void Propagator::predict_mean_analytic(std::shared_ptr<State> state, number_t dt, const Eigen::Matrix<number_t, 3, 1> &w_hat, const Eigen::Matrix<number_t, 3, 1> &a_hat,
                                       Eigen::Matrix<number_t, 4, 1> &new_q, Eigen::Matrix<number_t, 3, 1> &new_v, Eigen::Matrix<number_t, 3, 1> &new_p,
                                       Eigen::Matrix<number_t, 3, 18> &Xi_sum)
{

  // Pre-compute things
  Eigen::Matrix<number_t, 3, 3> R_Gtok = state->_imu->Rot();
  Eigen::Matrix<number_t, 4, 1> q_ktok1 = core::rot_2_quat(Xi_sum.block(0, 0, 3, 3));
  Eigen::Matrix<number_t, 3, 3> Xi_1 = Xi_sum.block(0, 3, 3, 3);
  Eigen::Matrix<number_t, 3, 3> Xi_2 = Xi_sum.block(0, 6, 3, 3);

  // Use our integrated Xi's to move the state forward
  new_q = core::quat_multiply(q_ktok1, state->_imu->quat());
...
void Propagator::compute_F_and_G_analytic(std::shared_ptr<State> state, number_t dt, const Eigen::Matrix<number_t,3,1> &w_hat,
                                          const Eigen::Matrix<number_t,3,1> &a_hat, const Eigen::Matrix<number_t,3,1> &w_uncorrected,
                                          const Eigen::Matrix<number_t,3,1> &a_uncorrected, const Eigen::Matrix<number_t, 4,1> &new_q, const Eigen::Matrix<number_t,3,1> &new_v,
                                          const Eigen::Matrix<number_t,3,1> &new_p, const Eigen::Matrix<number_t, 3, 18> &Xi_sum, MatrixX &F,
                                          MatrixX &G) {
  ...
  Eigen::Matrix<number_t, 3, 3> R_k = state->_imu->Rot();
  Eigen::Matrix<number_t, 3, 1> v_k = state->_imu->vel();
  Eigen::Matrix<number_t, 3, 1> p_k = state->_imu->pos();
  if (state->_options.do_fej) {
    R_k = state->_imu->Rot_fej();
    v_k = state->_imu->vel_fej();
    p_k = state->_imu->pos_fej();
  }
  Eigen::Matrix<number_t, 3, 3> dR_ktok1 = quat_2_Rot(new_q) * R_k.transpose();

  Eigen::Matrix<number_t, 3, 3> Dw = State::Dm(state->_options.imu_model, state->_calib_imu_dw->value());
  Eigen::Matrix<number_t, 3, 3> Da = State::Dm(state->_options.imu_model, state->_calib_imu_da->value());
  Eigen::Matrix<number_t, 3, 3> Tg = State::Tg(state->_calib_imu_tg->value());
  Eigen::Matrix<number_t, 3, 3> R_atoI = state->_calib_imu_ACCtoIMU->Rot();
  Eigen::Matrix<number_t, 3, 3> R_wtoI = state->_calib_imu_GYROtoIMU->Rot();
  Eigen::Matrix<number_t, 3, 1> a_k = R_atoI * Da * a_uncorrected;
  Eigen::Matrix<number_t, 3, 1> w_k = R_wtoI * Dw * w_uncorrected; // contains gravity correction already

  Eigen::Matrix<number_t, 3, 3> Xi_1 = Xi_sum.block(0, 3, 3, 3);
  Eigen::Matrix<number_t, 3, 3> Xi_2 = Xi_sum.block(0, 6, 3, 3);
  Eigen::Matrix<number_t, 3, 3> Jr_ktok1 = Xi_sum.block(0, 9, 3, 3);
  Eigen::Matrix<number_t, 3, 3> Xi_3 = Xi_sum.block(0, 12, 3, 3);
  Eigen::Matrix<number_t, 3, 3> Xi_4 = Xi_sum.block(0, 15, 3, 3);
  ...
  // for v
  F.block(p_id, v_id, 3, 3) = Eigen::Matrix<number_t, 3, 3>::Identity() * dt;
  ...
  if (Dw_id != -1) {
    MatrixX H_Dw = compute_H_Dw(state, w_uncorrected);
  ...
  if (Da_id != -1) {
    MatrixX H_Da = compute_H_Da(state, a_uncorrected);
  ...
  if (Tg_id != -1) {
    MatrixX H_Tg = compute_H_Tg(state, a_k);
  ...
  if (th_atoI_id != -1) {
    F.block(th_id, th_atoI_id, 3, 3) = -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * core::skew_x(a_k);
    F.block(p_id, th_atoI_id, 3, 3) = R_k.transpose() * (Xi_2 + Xi_4 * R_wtoI * Dw * Tg) * core::skew_x(a_k);
    F.block(v_id, th_atoI_id, 3, 3) = R_k.transpose() * (Xi_1 + Xi_3 * R_wtoI * Dw * Tg) * core::skew_x(a_k);
    F.block(th_atoI_id, th_atoI_id, 3, 3).setIdentity();
  }

  // begin to add the state transition matrix for the R_GYROtoIMU part
  if (th_wtoI_id != -1) {
    F.block(th_id, th_wtoI_id, 3, 3) = dR_ktok1 * Jr_ktok1 * dt * core::skew_x(w_k);
    F.block(p_id, th_wtoI_id, 3, 3) = -R_k.transpose() * Xi_4 * core::skew_x(w_k);
    F.block(v_id, th_wtoI_id, 3, 3) = -R_k.transpose() * Xi_3 * core::skew_x(w_k);
  ...
  G.block(bg_id, 6, 3, 3) = dt * Eigen::Matrix<number_t, 3, 3>::Identity();
  G.block(ba_id, 9, 3, 3) = dt * Eigen::Matrix<number_t, 3, 3>::Identity();
}

void Propagator::compute_F_and_G_discrete(std::shared_ptr<State> state, number_t dt, const Eigen::Matrix<number_t, 3, 1> &w_hat,
                                          const Eigen::Matrix<number_t, 3, 1> &a_hat, const Eigen::Matrix<number_t, 3, 1> &w_uncorrected,
                                          const Eigen::Matrix<number_t, 3, 1> &a_uncorrected, const Eigen::Matrix<number_t, 4, 1> &new_q, const Eigen::Matrix<number_t, 3, 1> &new_v,
                                          const Eigen::Matrix<number_t, 3, 1> &new_p, MatrixX &F, MatrixX &G)
{
  ...
  Eigen::Matrix<number_t, 3, 1> a_k = R_atoI * Da * a_uncorrected;
  Eigen::Matrix<number_t, 3, 1> w_k = R_wtoI * Dw * w_uncorrected; // contains gravity correction already
  Eigen::Matrix<number_t, 3, 3> Jr_ktok1 = Jr_so3(log_so3(dR_ktok1));
  ...
  if (th_atoI_id != -1) {
    F.block(th_id, th_atoI_id, 3, 3) = -dR_ktok1 * Jr_ktok1 * dt * R_wtoI * Dw * Tg * core::skew_x(a_k);
    F.block(p_id, th_atoI_id, 3, 3) = 0.5 * R_k.transpose() * dt * dt * core::skew_x(a_k);
    F.block(v_id, th_atoI_id, 3, 3) = R_k.transpose() * dt * core::skew_x(a_k);
    F.block(th_atoI_id, th_atoI_id, 3, 3).setIdentity();
  }
...
MatrixX Propagator::compute_H_Dw(std::shared_ptr<State> state, const Eigen::Matrix<number_t, 3, 1> &w_uncorrected)
{

  Eigen::Matrix<number_t, 3, 3> I_3x3 = MatrixX::Identity(3, 3);
  Eigen::Matrix<number_t, 3, 1> e_1 = I_3x3.block(0, 0, 3, 1);
  Eigen::Matrix<number_t, 3, 1> e_2 = I_3x3.block(0, 1, 3, 1);
  Eigen::Matrix<number_t, 3, 1> e_3 = I_3x3.block(0, 2, 3, 1);
  number_t w_1 = w_uncorrected(0);
  number_t w_2 = w_uncorrected(1);
  number_t w_3 = w_uncorrected(2);
  assert(state->_options.do_calib_imu_intrinsics);

  MatrixX H_Dw = MatrixX::Zero(3, 6);
  if (state->_options.imu_model == StateOptions::ImuModel::KALIBR) {
...
MatrixX Propagator::compute_H_Da(std::shared_ptr<State> state, const Eigen::Matrix<number_t, 3, 1> &a_uncorrected)
{
  ...
  Eigen::Matrix<number_t, 3, 1> e_2 = I_3x3.block(0, 1, 3, 1);
  Eigen::Matrix<number_t, 3, 1> e_3 = I_3x3.block(0, 2, 3, 1);
  number_t a_1 = a_uncorrected(0);
  number_t a_2 = a_uncorrected(1);
  number_t a_3 = a_uncorrected(2);
  assert(state->_options.do_calib_imu_intrinsics);

  MatrixX H_Da = MatrixX::Zero(3, 6);
  if (state->_options.imu_model == StateOptions::ImuModel::KALIBR) {
...
MatrixX Propagator::compute_H_Tg(std::shared_ptr<State> state, const Eigen::Matrix<number_t, 3, 1> &a_inI)
{

  Eigen::Matrix<number_t, 3, 3> I_3x3 = MatrixX::Identity(3, 3);
  number_t a_1 = a_inI(0);
  number_t a_2 = a_inI(1);
  number_t a_3 = a_inI(2);
  assert(state->_options.do_calib_imu_intrinsics);
  assert(state->_options.do_calib_imu_g_sensitivity);

  MatrixX H_Tg = MatrixX::Zero(3, 9);
