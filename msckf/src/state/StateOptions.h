...
struct StateOptions {
  ...
  type::LandmarkRepresentation::Representation feat_rep_msckf = type::LandmarkRepresentation::Representation::GLOBAL_3D;

  /// What representation our features are in (slam features)
  type::LandmarkRepresentation::Representation feat_rep_slam = type::LandmarkRepresentation::Representation::GLOBAL_3D;

  /// Nice print function of what parameters we have loaded
  void print(const std::shared_ptr<core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      std::string rep1 = type::LandmarkRepresentation::as_string(feat_rep_msckf);
      parser->parse_config("feat_rep_msckf", rep1);
      feat_rep_msckf = type::LandmarkRepresentation::from_string(rep1);
      std::string rep2 = type::LandmarkRepresentation::as_string(feat_rep_slam);
      parser->parse_config("feat_rep_slam", rep2);
      feat_rep_slam = type::LandmarkRepresentation::from_string(rep2);
    ...
    PRINT_DEBUG("  - feat_rep_msckf: %s\n", type::LandmarkRepresentation::as_string(feat_rep_msckf).c_str());
    PRINT_DEBUG("  - feat_rep_slam: %s\n", type::LandmarkRepresentation::as_string(feat_rep_slam).c_str());
  }
};
