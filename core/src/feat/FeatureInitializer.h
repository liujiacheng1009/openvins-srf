...
public:
  ...
  struct ClonePose {
    ...
    /// Rotation
    Eigen::Matrix<number_t, 3, 3> _Rot;

    /// Position
    Eigen::Matrix<number_t, 3, 1> _pos;

    /// Constructs pose from rotation and position
    ClonePose(const Eigen::Matrix<number_t, 3, 3> &R, const Eigen::Matrix<number_t, 3, 1> &p) {
      _Rot = R;
      _pos = p;
    }

    /// Constructs pose from quaternion and position
    ClonePose(const Eigen::Matrix<number_t, 4, 1> &q, const Eigen::Matrix<number_t, 3, 1> &p) {
      _Rot = quat_2_Rot(q);
      _pos = p;
    }
    ...
    /// Default constructor
    ClonePose() {
      _Rot = Eigen::Matrix<number_t, 3, 3>::Identity();
      _pos = Eigen::Matrix<number_t, 3, 1>::Zero();
    }

    /// Accessor for rotation
    const Eigen::Matrix<number_t, 3, 3> &Rot() { return _Rot; }

    /// Accessor for position
    const Eigen::Matrix<number_t, 3, 1> &pos() { return _pos; }
  };
