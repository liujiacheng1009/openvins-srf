...
bool FeatureDatabase::determine_keyframe(std::vector<size_t> ids, size_t cam_id, double last_kf_timestamp)
{
  constexpr int kTrackNumThreshold = 60;
  constexpr double kTrackRatioThreshold = 0.5;
  constexpr double kAvgFlowNormThreshold = 30.;

  int track_num{0};
  double avg_flow_norm{0.};
  std::lock_guard<std::mutex> lck(mtx);
  for (auto id : ids)
  {
    if (features_idlookup.find(id) != features_idlookup.end())
    {
      std::shared_ptr<Feature> feat = features_idlookup.at(id);
      auto it0 = std::find(feat->timestamps[cam_id].begin(), feat->timestamps[cam_id].end(), last_kf_timestamp);
      if (it0 == feat->timestamps[cam_id].end())
        continue;
      auto idx0 = std::distance(feat->timestamps[cam_id].begin(), it0);
      avg_flow_norm += (feat->uvs[cam_id][idx0] - feat->uvs[cam_id].back()).norm();
      track_num++;
    }
  }

  double track_ratio = (double)track_num / ids.size();
  avg_flow_norm /= track_num;

  return track_num < kTrackNumThreshold || track_ratio < kTrackRatioThreshold || avg_flow_norm > kAvgFlowNormThreshold;
}

std::pair<int, double> FeatureDatabase::tracking_info_from_lastkf(size_t cam_id, double curr_frm_timestamp, double last_kf_timestamp)
{
  double tracking_ratio = 0.;
  std::lock_guard<std::mutex> lck(mtx);
  int feat_last_kf = 0, feat_curr_frm = 0;
  for (auto it = features_idlookup.begin(); it != features_idlookup.end(); ++it)
  {
    if (std::find(it->second->timestamps[cam_id].begin(), it->second->timestamps[cam_id].end(), last_kf_timestamp) == it->second->timestamps[cam_id].end())
      continue;
    feat_last_kf++;
    auto feat = it->second;
    if (std::find(feat->timestamps[cam_id].begin(), feat->timestamps[cam_id].end(), curr_frm_timestamp) != feat->timestamps[cam_id].end())
      feat_curr_frm++;
  }
  if (feat_last_kf != 0)
  {
    tracking_ratio = feat_curr_frm / (1.0 * feat_last_kf);
  }
  return std::make_pair(feat_curr_frm, tracking_ratio);
}
