...
bool FeatureInitializer::single_triangulation(std::shared_ptr<Feature> feat,
  ...
  Eigen::Matrix<number_t, 3, 3> A = Eigen::Matrix<number_t, 3, 3>::Zero();
  Eigen::Matrix<number_t, 3, 1> b = Eigen::Matrix<number_t, 3, 1>::Zero();

  // Get the position of the anchor pose
  ClonePose anchorclone = clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp);
  const Eigen::Matrix<number_t, 3, 3> &R_GtoA = anchorclone.Rot();
  const Eigen::Matrix<number_t, 3, 1> &p_AinG = anchorclone.pos();
  ...
  for (auto const &pair : feat->timestamps) {
    ...
    for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
      ...
      const Eigen::Matrix<number_t, 3, 3> &R_GtoCi = clonesCAM.at(pair.first).at(feat->timestamps.at(pair.first).at(m)).Rot();
      const Eigen::Matrix<number_t, 3, 1> &p_CiinG = clonesCAM.at(pair.first).at(feat->timestamps.at(pair.first).at(m)).pos();

      // Convert current position relative to anchor
      Eigen::Matrix<number_t, 3, 3> R_AtoCi;
      R_AtoCi.noalias() = R_GtoCi * R_GtoA.transpose();
      Eigen::Matrix<number_t, 3, 1> p_CiinA;
  ...
  MatrixX p_f = A.colPivHouseholderQr().solve(b);
...
bool FeatureInitializer::single_triangulation_1d(std::shared_ptr<Feature> feat,
  ...
  Eigen::Matrix<number_t, 3, 1> bearing_inA;
  ...
  for (auto const &pair : feat->timestamps) {
    ...
    for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
      ...
      // Append to our linear system
      Eigen::Matrix<number_t, 3, 1> BperpBanchor = Bperp * bearing_inA;
...
bool FeatureInitializer::single_gaussnewton(std::shared_ptr<Feature> feat,
  ...
  bool recompute = true;
  Eigen::Matrix<number_t, 3, 3> Hess = Eigen::Matrix<number_t, 3, 3>::Zero();
  Eigen::Matrix<number_t, 3, 1> grad = Eigen::Matrix<number_t, 3, 1>::Zero();

  // Cost at the last iteration
  double cost_old = compute_error(clonesCAM, feat, alpha, beta, rho);

  // Get the position of the anchor pose
  const Eigen::Matrix<number_t, 3, 3> &R_GtoA = clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp).Rot();
  const Eigen::Matrix<number_t, 3, 1> &p_AinG = clonesCAM.at(feat->anchor_cam_id).at(feat->anchor_clone_timestamp).pos();
  ...
  while (runs < _options.max_runs && lam < _options.max_lamda && eps > _options.min_dx) {
    ...
    if (recompute) {
      ...
      for (auto const &pair : feat->timestamps) {
        ...
        for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
          ...
          // Get the position of this clone in the global
          const Eigen::Matrix<number_t, 3, 3> &R_GtoCi = clonesCAM.at(pair.first).at(feat->timestamps[pair.first].at(m)).Rot();
          const Eigen::Matrix<number_t, 3, 1> &p_CiinG = clonesCAM.at(pair.first).at(feat->timestamps[pair.first].at(m)).pos();
          // Convert current position relative to anchor
          Eigen::Matrix<number_t, 3, 3> R_AtoCi;
          R_AtoCi.noalias() = R_GtoCi * R_GtoA.transpose();
          Eigen::Matrix<number_t, 3, 1> p_CiinA;
          p_CiinA.noalias() = R_GtoA * (p_CiinG - p_AinG);
          Eigen::Matrix<number_t, 3, 1> p_AinCi;
          ...
          number_t hi1 = R_AtoCi(0, 0) * alpha + R_AtoCi(0, 1) * beta + R_AtoCi(0, 2) + rho * p_AinCi(0, 0);
          number_t hi2 = R_AtoCi(1, 0) * alpha + R_AtoCi(1, 1) * beta + R_AtoCi(1, 2) + rho * p_AinCi(1, 0);
          number_t hi3 = R_AtoCi(2, 0) * alpha + R_AtoCi(2, 1) * beta + R_AtoCi(2, 2) + rho * p_AinCi(2, 0);
          // Calculate jacobian
          number_t d_z1_d_alpha = (R_AtoCi(0, 0) * hi3 - hi1 * R_AtoCi(2, 0)) / (pow(hi3, 2));
          number_t d_z1_d_beta = (R_AtoCi(0, 1) * hi3 - hi1 * R_AtoCi(2, 1)) / (pow(hi3, 2));
          number_t d_z1_d_rho = (p_AinCi(0, 0) * hi3 - hi1 * p_AinCi(2, 0)) / (pow(hi3, 2));
          number_t d_z2_d_alpha = (R_AtoCi(1, 0) * hi3 - hi2 * R_AtoCi(2, 0)) / (pow(hi3, 2));
          number_t d_z2_d_beta = (R_AtoCi(1, 1) * hi3 - hi2 * R_AtoCi(2, 1)) / (pow(hi3, 2));
          number_t d_z2_d_rho = (p_AinCi(1, 0) * hi3 - hi2 * p_AinCi(2, 0)) / (pow(hi3, 2));
          Eigen::Matrix<number_t, 2, 3> H;
          ...
          grad.noalias() += H.transpose() * res.cast<number_t>();
    ...
    Eigen::Matrix<number_t, 3, 3> Hess_l = Hess;
    for (size_t r = 0; r < (size_t)Hess.rows(); r++)
    {
      Hess_l(r, r) *= (1.0 + lam);
    }

    Eigen::Matrix<number_t, 3, 1> dx = Hess_l.colPivHouseholderQr().solve(grad);
  ...
  Eigen::HouseholderQR<MatrixX> qr(feat->p_FinA);
  MatrixX Q = qr.householderQ();
  ...
  for (auto const &pair : feat->timestamps) {
    ...
    for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
      ...
      const Eigen::Matrix<number_t, 3, 1> &p_CiinG = clonesCAM.at(pair.first).at(feat->timestamps.at(pair.first).at(m)).pos();
      // Convert current position relative to anchor
      Eigen::Matrix<number_t, 3, 1> p_CiinA = R_GtoA * (p_CiinG - p_AinG);
