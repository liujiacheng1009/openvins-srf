...
inline Eigen::Matrix<typename Derived::Scalar, 4, 1> rot_2_quat(const Eigen::MatrixBase<Derived> &rot) {
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3> skew_x(const Eigen::MatrixBase<Derived> &w) {
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 3, 3> w_x;
  w_x << 0, -w(2,0), w(1,0), w(2,0), 0, -w(0,0), -w(1,0), w(0,0), 0;
  return w_x;
}
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3> quat_2_Rot(const Eigen::MatrixBase<Derived> &q)
{
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 3, 3> q_x = skew_x(q.block(0, 0, 3, 1));
  Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> Rot = (2 * std::pow(q(3, 0), 2) - 1) * Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Identity(3, 3) - 2 * q(3, 0) * q_x + 2 * q.block(0, 0, 3, 1) * (q.block(0, 0, 3, 1).transpose());
  return Rot;
}
...
inline Eigen::Matrix<typename Derived1::Scalar, 4, 1> quat_multiply(const Eigen::MatrixBase<Derived1> &q, const Eigen::MatrixBase<Derived2> &p) {
  using Scalar = typename Derived1::Scalar;
  Eigen::Matrix<Scalar, 4, 1> q_t;
  Eigen::Matrix<Scalar, 4, 4> Qm;
  // create big L matrix
  Qm.block(0, 0, 3, 3) = q(3, 0) * Eigen::Matrix<Scalar, Eigen::Dynamic,Eigen::Dynamic >::Identity(3, 3) - skew_x(q.block(0, 0, 3, 1));
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 1> vee(const Eigen::MatrixBase<Derived> &w_x) {
  using Scalar = typename Derived::Scalar;
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3> exp_so3(const Eigen::MatrixBase<Derived> &w) {
  // get theta
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 3, 3> w_x = skew_x(w);
  Scalar theta = w.norm();
  ...
  if (theta == 0) {
    R = Eigen::Matrix<Scalar, Eigen::Dynamic,Eigen::Dynamic >::Identity(3, 3);
  } else {
    R = Eigen::Matrix<Scalar, Eigen::Dynamic,Eigen::Dynamic >::Identity(3, 3) + A * w_x + B * w_x * w_x;
  }
  return R;
}
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 1> log_so3(const Eigen::MatrixBase<Derived> &R) {
  using Scalar = typename Derived::Scalar;
  // note switch to base 1
  Scalar R11 = R(0, 0), R12 = R(0, 1), R13 = R(0, 2);
  Scalar R21 = R(1, 0), R22 = R(1, 1), R23 = R(1, 2);
  Scalar R31 = R(2, 0), R32 = R(2, 1), R33 = R(2, 2);
  ...
  if (tr + 1.0 < 1e-10) {
    if (std::abs(R33 + 1.0) > 1e-5)
      omega = (M_PI / sqrt(2.0 + 2.0 * R33)) * Eigen::Matrix<Scalar, 3,1>(R13, R23, 1.0 + R33);
    else if (std::abs(R22 + 1.0) > 1e-5)
      omega = (M_PI / sqrt(2.0 + 2.0 * R22)) * Eigen::Matrix<Scalar, 3,1>(R12, 1.0 + R22, R32);
    else
      // if(std::abs(R.r1_.x()+1.0) > 1e-5)  This is implicit
      omega = (M_PI / sqrt(2.0 + 2.0 * R11)) * Eigen::Matrix<Scalar, 3,1>(1.0 + R11, R21, R31);
  } else {
    Scalar magnitude;
    const Scalar tr_3 = tr - 3.0; // always negative
    if (tr_3 < -1e-7) {
      Scalar theta = acos((tr - 1.0) / 2.0);
    ...
    omega = magnitude * Eigen::Matrix<Scalar, 3,1>(R32 - R23, R13 - R31, R21 - R12);
  }
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4,4> exp_se3(const Eigen::MatrixBase<Derived>& vec) {
  using Scalar = typename Derived::Scalar;
  // Precompute our values
  Eigen::Matrix<Scalar, 3,1> w = vec.head(3);
  Eigen::Matrix<Scalar, 3,1> u = vec.tail(3);
  Scalar theta = sqrt(w.dot(w));
  Eigen::Matrix<Scalar, 3,3> wskew;
  ...
  Eigen::Matrix<Scalar, 3,3> I_33 = Eigen::Matrix<Scalar, 3,3>::Identity();
  Eigen::Matrix<Scalar, 3,3> V = I_33 + B * wskew + C * wskew * wskew;
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 6, 1> log_se3(const Eigen::MatrixBase<Derived> &mat) {
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 3,1> w = log_so3(mat.block(0, 0, 3,3));
  Eigen::Matrix<Scalar, 3,1> T = mat.block(0, 3,3,1);
  const Scalar t = w.norm();
  if (t < 1e-10) {
    Eigen::Matrix<Scalar, 6, 1> log;
    log << w, T;
    return log;
  } else {
    Eigen::Matrix<Scalar, 3,3> W = skew_x(w / t);
    // Formula from Agrawal06iros, equation (14)
    // simplified with Mathematica, and multiplying in T to avoid matrix math
    Scalar Tan = tan(0.5 * t);
    Eigen::Matrix<Scalar, 3,1> WT = W * T;
    Eigen::Matrix<Scalar, 3,1> u = T - (0.5 * t) * WT + (1 - t / (2. * Tan)) * (W * WT);
    Eigen::Matrix<Scalar, 6, 1> log;
    log << w, u;
    return log;
  }
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4,4> hat_se3(const Eigen::MatrixBase<Derived> &vec) {
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 4,4> mat = Eigen::Matrix<Scalar, 4,4>::Zero();
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4,4> Inv_se3(const Eigen::MatrixBase<Derived> &T) {
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 4,4> Tinv = Eigen::Matrix<Scalar, 4,4>::Identity();
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 1> Inv(Eigen::MatrixBase<Derived> q) {
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 4, 1> qinv;
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 4> Omega(const Eigen::MatrixBase<Derived>& w) {
  using Scalar = typename Derived::Scalar;
 ...
 */
template <class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 1> quatnorm(const Eigen::MatrixBase<Derived> &q_t)
{
  using Scalar = typename Derived::Scalar;
  Eigen::Matrix<Scalar, 4, 1> q = q_t;
  if (q_t(3, 0) < 0)
  {
    q = -q_t;
  }
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3> Jl_so3(const Eigen::MatrixBase<Derived> &w) {
  using Scalar = typename Derived::Scalar;
  Scalar theta = w.norm();
  if (theta < 1e-6) {
    return Eigen::Matrix<Scalar, Eigen::Dynamic,Eigen::Dynamic >::Identity(3, 3);
  } else {
    Eigen::Matrix<Scalar, 3, 1> a = w / theta;
    Eigen::Matrix<Scalar, 3, 3> J = sin(theta) / theta * Eigen::Matrix<Scalar, Eigen::Dynamic,Eigen::Dynamic >::Identity(3, 3) + (1 - sin(theta) / theta) * a * a.transpose() +
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3> Jr_so3(const Eigen::MatrixBase<Derived> &w) { return Jl_so3(-w); }
 ...
 */
template<class Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 1> rot2rpy(const Eigen::MatrixBase<Derived> &rot) {
  using Scalar = typename Derived::Scalar;
...
inline Eigen::Matrix<Scalar, 3, 3> rot_x(Scalar t) {
  Eigen::Matrix<Scalar, 3, 3> r;
  Scalar ct = cos(t);
  Scalar st = sin(t);
 ...
 */
template<class Scalar>
inline Eigen::Matrix<Scalar, 3, 3> rot_y(Scalar t) {
  Eigen::Matrix<Scalar, 3, 3> r;
  Scalar ct = cos(t);
  Scalar st = sin(t);
 ...
 */
template<class Scalar>
inline Eigen::Matrix<Scalar, 3, 3> rot_z(Scalar t) {
  Eigen::Matrix<Scalar, 3, 3> r;
  Scalar ct = cos(t);
  Scalar st = sin(t);
