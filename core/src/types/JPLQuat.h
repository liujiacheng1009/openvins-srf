...
public:
  JPLQuat() : Type(3) {
    Eigen::Matrix<number_t, 4, 1> q0 = Eigen::Matrix<number_t, 4, 1>::Zero();
    q0(3) = 1.0;
  ...
  void update(const VectorX &dx) override {
    ...
    set_value(core::quat_multiply(dq, _value));
  }
  ...
  /// Rotation access
  Eigen::Matrix<number_t, 3, 3> Rot() const { return _R; }

  /// FEJ Rotation access
  Eigen::Matrix<number_t, 3, 3> Rot_fej() const { return _Rfej; }

protected:
  // Stores the rotation
  Eigen::Matrix<number_t, 3, 3> _R;

  // Stores the first-estimate rotation
  Eigen::Matrix<number_t, 3, 3> _Rfej;
  ...
  void set_fej_internal(const MatrixX &new_value) {
    ...
    // compute associated rotation
    _Rfej = core::quat_2_Rot(new_value);
  }
};
