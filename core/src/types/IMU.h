...
public:
  IMU() : Type(15) {
    ...
    VectorX imu0 = VectorX::Zero(16, 1);
    imu0(3) = 1.0;
   ...
   */
  void update(const VectorX &dx) override {

    assert(dx.rows() == _size);

    Eigen::Matrix<number_t, 16, 1> newX = _value;

    Eigen::Matrix<number_t, 4, 1> dq;
    dq << .5 * dx.block(0, 0, 3, 1), 1.0;
    dq = core::quatnorm(dq);

    newX.block(0, 0, 4, 1) = core::quat_multiply(dq, quat());
   ...
   */
  void set_value(const MatrixX &new_value) override { set_value_internal(new_value); }

  /**
   * @brief Sets the value of the first estimate
   * @param new_value New value we should set
   */
  void set_fej(const MatrixX &new_value) override { set_fej_internal(new_value); }
  ...
  /// Rotation access
  Eigen::Matrix<number_t, 3, 3> Rot() const { return _pose->Rot(); }

  /// FEJ Rotation access
  Eigen::Matrix<number_t, 3, 3> Rot_fej() const { return _pose->Rot_fej(); }

  /// Rotation access quaternion
  Eigen::Matrix<number_t, 4, 1> quat() const { return _pose->quat(); }

  /// FEJ Rotation access quaternion
  Eigen::Matrix<number_t, 4, 1> quat_fej() const { return _pose->quat_fej(); }

  /// Position access
  Eigen::Matrix<number_t, 3, 1> pos() const { return _pose->pos(); }

  /// FEJ position access
  Eigen::Matrix<number_t, 3, 1> pos_fej() const { return _pose->pos_fej(); }

  /// Velocity access
  Eigen::Matrix<number_t, 3, 1> vel() const { return _v->value(); }

  // FEJ velocity access
  Eigen::Matrix<number_t, 3, 1> vel_fej() const { return _v->fej(); }

  /// Gyro bias access
  Eigen::Matrix<number_t, 3, 1> bias_g() const { return _bg->value(); }

  /// FEJ gyro bias access
  Eigen::Matrix<number_t, 3, 1> bias_g_fej() const { return _bg->fej(); }

  /// Accel bias access
  Eigen::Matrix<number_t, 3, 1> bias_a() const { return _ba->value(); }

  // FEJ accel bias access
  Eigen::Matrix<number_t, 3, 1> bias_a_fej() const { return _ba->fej(); }
...
protected:
   ...
   * @param new_value New value we should set
   */
  void set_value_internal(const MatrixX &new_value) {
   ...
   * @param new_value New value we should set
   */
  void set_fej_internal(const MatrixX &new_value) {
