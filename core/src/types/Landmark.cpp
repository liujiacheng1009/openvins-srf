...
Eigen::Matrix<number_t, 3, 1> Landmark::get_xyz(bool getfej) const {
  ...
  if (_feat_representation == LandmarkRepresentation::Representation::GLOBAL_FULL_INVERSE_DEPTH ||
    ...
    Eigen::Matrix<number_t, 3, 1> p_invFinG = (getfej) ? fej() : value();
  ...
  if (_feat_representation == LandmarkRepresentation::Representation::ANCHORED_MSCKF_INVERSE_DEPTH) {
    ...
    Eigen::Matrix<number_t, 3, 1> p_invFinA = value();
  ...
  // Failure
  assert(false);
  return Eigen::Matrix<number_t, 3, 1>::Zero();
}

void Landmark::set_from_xyz(Eigen::Matrix<number_t, 3, 1> p_FinG, bool isfej) {
  ...
  if (_feat_representation == LandmarkRepresentation::Representation::GLOBAL_FULL_INVERSE_DEPTH ||
    ...
    number_t g_rho = 1 / p_FinG.norm();
    number_t g_phi = std::acos(g_rho * p_FinG(2));
    // number_t g_theta = std::asin(g_rho*p_FinG(1)/std::sin(g_phi));
    number_t g_theta = std::atan2(p_FinG(1), p_FinG(0));
    Eigen::Matrix<number_t, 3, 1> p_invFinG;
    p_invFinG(0) = g_theta;
    p_invFinG(1) = g_phi;
    p_invFinG(2) = g_rho;
  ...
  if (_feat_representation == LandmarkRepresentation::Representation::ANCHORED_MSCKF_INVERSE_DEPTH) {
    ...
    Eigen::Matrix<number_t, 3, 1> p_invFinA_MSCKF;
