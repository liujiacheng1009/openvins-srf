...
int main(int argc, char **argv) {
  ...
  // Load parameters
  auto parser = std::make_shared<core::YamlParser>(config_path, false);
  ...
  // Histogram method
  core::TrackBase::HistogramMethod method;
  ...
  if (histogram_method_str == "NONE") {
    method = core::TrackBase::NONE;
  } else if (histogram_method_str == "HISTOGRAM") {
    method = core::TrackBase::HISTOGRAM;
  } else if (histogram_method_str == "CLAHE") {
    method = core::TrackBase::CLAHE;
  } else {
  ...
  for (int i = 0; i < 2; i++) {
    Eigen::Matrix<number_t, 8, 1> cam0_calib;
  ...
  extractor = new TrackKLT(cameras, num_pts, !use_stereo, method, fast_threshold, grid_x, grid_y, min_px_dist);
  // extractor = new TrackDescriptor(cameras, num_pts, !use_stereo, method, fast_threshold, grid_x, grid_y, min_px_dist,
  // knn_ratio); 
