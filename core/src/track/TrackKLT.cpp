...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  if (pts_last[cam_id].empty()) {
    ...
    is_curr_keyframe = true;
    last_keyframe_ts = message.timestamp;
    return;
  }
  ...
  is_curr_keyframe = database->determine_keyframe(good_ids_left, cam_id, last_keyframe_ts);
  static int last_is_curr_keyframe = -1;
  if (last_is_curr_keyframe != static_cast<int>(is_curr_keyframe)) {
    LOG(INFO) << "is_curr_keyframe " << static_cast<int>(is_curr_keyframe);
    last_is_curr_keyframe = static_cast<int>(is_curr_keyframe);
  }
  if (is_curr_keyframe)
    last_keyframe_ts = message.timestamp;
  ...
  ShowFeatTrackResult(img_pyramid_last[cam_id][0], img_pyramid_curr[cam_id][0],
                      pts_left_old, pts_left_new, mask_ll,
                      pts_left_old, "left track", true, 1.0);
}
