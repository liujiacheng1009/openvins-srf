...
public:
   ...
   */
  explicit TrackDescriptor(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, bool stereo,
                           HistogramMethod histmethod, int fast_threshold, int gridx, int gridy, int minpxdist, double knnratio)
      : TrackBase(cameras, numfeats, stereo, histmethod), threshold(fast_threshold), grid_x(gridx), grid_y(gridy),
